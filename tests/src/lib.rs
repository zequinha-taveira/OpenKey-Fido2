...
fn test_crypto_encrypt_decrypt_roundtrip() {
    ...
    // lgtm[rust/hard-coded-cryptographic-value] — nonce fixo apenas para teste de roundtrip.
    let nonce = [0u8; 12];
...
fn test_crypto_encrypt_tampered_ciphertext() {
    let crypto = CryptoEngine::new().unwrap();
    let plaintext = b"secret message";
    // lgtm[rust/hard-coded-cryptographic-value] — nonce fixo apenas para teste de tampering.
    let nonce = [0u8; 12];
...
fn test_device_profile_builder_from_board() {
    use authenticator::{DeviceProfileBuilder, Transport};
    use board_generic::BoardDefinition;
...
fn test_device_profile_builder_overrides() {
    use authenticator::{AttestationType, DeviceProfileBuilder, Extension, PinPolicy};
...
fn test_capability_discovery_reports_runtime() {
    use authenticator::{CapabilityDiscovery, DeviceProfileBuilder, PinPolicy};
...
fn test_embedded_authenticator_capabilities_drive_get_info() {
    use authenticator::EmbeddedAuthenticator;
    use authenticator::{DeviceProfileBuilder, Extension, PinPolicy};
...
fn test_board_profiles_derive_correct_product_name_and_transports() {
    use authenticator::{DeviceProfileBuilder, Transport};
    use profile_generic::GENERIC;
    use profile_rp2350::{RP2350, RP2350_ZERO};
    use profile_stm32::STM32L4;
...
fn test_embedded_authenticator_with_rp2350_zero_profile() {
    use authenticator::EmbeddedAuthenticator;
    use profile_rp2350::RP2350_ZERO;
...
fn test_board_profiles_have_unique_aaguids() {
    use profile_generic::GENERIC;
    use profile_rp2350::{RP2350, RP2350_ZERO, YUBIKEY_4_5};
    use profile_stm32::STM32L4;
...
fn test_transport_config_usb_hid() {
    use authenticator::{DeviceProfileBuilder, TransportConfig, TransportType};
...
fn test_embedded_authenticator_with_transport() {
    use authenticator::EmbeddedAuthenticator;
    use authenticator::{DeviceProfileBuilder, TransportConfig};
...
fn test_embedded_authenticator_without_transport() {
    use authenticator::DeviceProfileBuilder;
    use authenticator::EmbeddedAuthenticator;
...
mod embedded_authenticator_transport_tests {
    ...
    fn usb_hid_profile() -> authenticator::DeviceProfile {
        DeviceProfileBuilder::new()
            .transport_config(TransportConfig::usb_hid())
            .build()
    }

    fn usb_ccid_profile() -> authenticator::DeviceProfile {
        DeviceProfileBuilder::new()
            .transport_config(TransportConfig::usb_ccid())
            .build()
    }
...
fn test_multiprotocol_router_piv_verify_generate_auth() {
    use authenticator::firmware::AID_PIV;
...
fn test_multiprotocol_router_openpgp_verify_generate_sign() {
    use authenticator::firmware::AID_OPENPGP;

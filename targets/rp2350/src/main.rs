...
use openkey_core::ctap2::{Ctap2Authenticator, Ctap2Error};
...
const _: () = assert!(crate::profile::YUBIKEY_4_5.led_pin == 16);
...
fn main() -> ! {
    ...
    let aaguid = crate::profile::YUBIKEY_4_5.aaguid;
    #[cfg(not(feature = "yubikey5-identity"))]
    let aaguid = crate::profile::RP2350_ZERO.aaguid;

    let ctap_storage = StorageEngine::new().expect("ctap storage");
    let mut ctap2_auth =
        Ctap2Authenticator::new(aaguid, crypto.clone(), ctap_storage).expect("ctap2 authenticator");
        ...
        CtaphidCommand::Cbor => {
            if msg.payload.is_empty() {
                ...
                match ctap2_auth.process_command(cmd_byte, cbor_data) {
                    ...
                    Err(err) => (msg.cid, CtaphidCommand::Cbor, alloc::vec![err.as_u8()]),
                }
            }
        }
        CtaphidCommand::Msg => (

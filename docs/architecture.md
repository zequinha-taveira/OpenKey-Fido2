...
- **Contrato**: encryption at rest via ChaCha20-Poly1305. `StorageBackend`
  ...
  (`warn`-only, threshold 10k com throttle a cada 1000, sem rotação real;
   ver `crates/openkey-storage/src/storage.rs` e `ADR-0016`). Estado
...
|-------|---------|-----------|------------------|
| `openkey-device` | `crates/openkey-device/` | device | API final, configuração e applets |
| `openkey-core` | `crates/openkey-core/` | core | CTAP2, WebAuthn e domínio MCU-independent |
| `openkey-crypto` | `crates/openkey-crypto/` | core | Primitivas criptográficas |
| `openkey-storage` | `crates/openkey-storage/` | core | Persistência cifrada |
| `openkey-transport` | `crates/openkey-transport/` | transport | HID, CCID, NFC, BLE e CTAPHID |
| `openkey-hal` | `crates/openkey-hal/` | hal | Contratos de hardware |
| `openkey-target-*` | `targets/*/` | targets | Integrações e perfis por board |
| `openkey-simulator` | `targets/simulator/` | targets | Simulador host |
...
| Novo transporte | `crates/openkey-transport/src/<meio>/` | Implementar `Transport` trait |
| Novo board | `targets/profile-<board>/` (nova crate `openkey-profile-*`) | `BoardDefinition` com `const fn` |

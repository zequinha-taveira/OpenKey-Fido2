...
fn canonicalize_cbor(value: Value) -> Value {
    match value {
        ...
        Value::Array(entries) => Value::Array(entries.into_iter().map(canonicalize_cbor).collect()),
        other => other,
    }
}
    ...
    match credential.algorithm {
        -7 => {
            let (x, y) = if credential.public_key.len() == 65 && credential.public_key[0] == 0x04 {
                (
                    &credential.public_key[1..33],
                    &credential.public_key[33..65],
                )
            } else if credential.public_key.len() == 64 {
                (
                    &credential.public_key[0..32],
                    &credential.public_key[32..64],
                )
            } else {
        ...
        _ => Value::Map(vec![
            ...
            (
                ...
                Value::Bytes(credential.public_key.clone()),
            ),
        ]),
    }
}
...
mod tests {
    ...
    fn test_make_credential_minimal_request_omits_optional_fields() {
        ...
        let req: MakeCredentialRequest =
            decode_cbor(&raw).expect("deve decodificar sem excludeList e options");
        ...
        for window in entries.windows(2) {
            ...
            assert!(
                a_bytes.len() < b_bytes.len()
                    || (a_bytes.len() == b_bytes.len() && a_bytes < b_bytes),
    ...
    impl PlatformSide {
        ...
        fn decrypt_output(&self, value: Option<Value>) -> Vec<u8> {
            match value.expect("saída hmac-secret ausente") {
                ...
                _ => panic!("saída inesperada da extensão: tipo CBOR inesperado"),
            }
        }
    }
    ...
    fn test_hmac_secret_fresh_nonce_per_request() {
        ...
        let enc_first = match out_first.extensions.unwrap().hmac_secret.unwrap() {
            Value::Bytes(v) => v,
            _ => panic!("saída inesperada: tipo CBOR inesperado"),
        };

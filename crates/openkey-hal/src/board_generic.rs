...
/// Falha genérica reportada por um dispositivo de verificação de usuário.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum UserVerificationError {
    /// A verificação local não foi satisfeita.
    VerificationFailed,
}

/// Interface mínima de um sensor de verificação de usuário embutida.
///
/// O crate de boards não depende de CTAP2: drivers concretos de biometria ou
/// de PIN-pad implementam este contrato e o crate `authenticator` faz a ponte
/// para `ctap2::UserVerification`. O dispositivo deve manter internamente o
/// contador de tentativas que sobreviva a power cycles quando isso for
/// exigido pelo hardware.
pub trait UserVerificationDevice: core::fmt::Debug + Send + Sync {
    /// Executa uma verificação física ou local do usuário.
    ///
    /// `Ok(())` indica sucesso; `Err(UserVerificationError)` indica falha. O
    /// autenticador converte a falha para o erro CTAP2 apropriado e persiste
    /// seu próprio contador de tentativas.
    fn verify_user(&mut self) -> Result<(), UserVerificationError>;

    /// Retorna as tentativas restantes informadas pelo dispositivo.
    fn retries(&self) -> u8;
}

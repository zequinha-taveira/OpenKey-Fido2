...
use super::ble_gatt::{BLE_CMD_MSG, BLE_HEADER_LEN, BLE_MAX_MESSAGE_LEN, BLE_MAX_NOTIFICATION_LEN};
use alloc::string::ToString;
use alloc::vec::Vec;
#[cfg(feature = "embedded")]
use transport_core::embedded::BleGattDevice;
...
mod tests {
    ...
    #[test]
    fn test_nrf52840_ble_composes_with_framed_transport() {
        use transport_core::embedded::nrf52840::Nrf52840BleGatt;

        let mut t = FramedBleGattTransport::new(Nrf52840BleGatt::new());
        t.init().unwrap();
        assert!(t.is_initialized());
        assert!(matches!(t.recv(), Err(TransportError::RecvError(_))));
        t.close().unwrap();

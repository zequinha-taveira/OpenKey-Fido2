...
use super::ble_gatt::BleGattDevice;
...
impl<P: OutputPin> Nrf52840UsbHid<'_, P> {
...
impl<P: OutputPin> UsbHidDevice for Nrf52840UsbHid<'_, P> {
...
/// Nordic nRF52840 BLE GATT reference implementation for the FIDO service.
pub struct Nrf52840BleGatt {
    initialized: bool,
    connected: bool,
}

impl Nrf52840BleGatt {
    /// Creates a BLE GATT device with advertising stopped.
    pub fn new() -> Self {
        Self {
            initialized: false,
            connected: false,
...
impl Default for Nrf52840BleGatt {
    fn default() -> Self {
        Self::new()
    }
}

impl BleGattDevice for Nrf52840BleGatt {
    fn init(&mut self) -> Result<(), EmbeddedTransportError> {
        // Real implementation:
        // 1. Initialize the SoftDevice/controller and GAP advertising.
        // 2. Register the FIDO service and its control-point/status chars.
        // 3. Enable write and notification events for the active connection.
        self.initialized = true;
        self.connected = true;
        Ok(())
    }
    ...
    fn send_notification(&mut self, data: &[u8]) -> Result<(), EmbeddedTransportError> {
        ...
        // Real implementation writes the notification through the GATT server.
        Ok(())
    }

    fn recv_command(&mut self, _buf: &mut [u8]) -> Result<usize, EmbeddedTransportError> {
        if !self.initialized {
            return Err(EmbeddedTransportError::NotInitialized);
        }
        if !self.connected {
        ...
        // Real implementation waits for the next control-point write event.
        Err(EmbeddedTransportError::Timeout)
    }

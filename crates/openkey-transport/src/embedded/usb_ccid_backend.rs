...
use super::{EmbeddedTransportError, UsbCcidDevice};
...
/// Adaptador do backend `usb-device` para a abstração de hardware CCID.
///
/// O backend já trata os cabeçalhos CCID e expõe APDUs no modo manual; esta
/// ponte permite reutilizar o mesmo driver com [`super::super::FramedCcidTransport`]
/// e, portanto, com a trait [`super::super::Transport`].
pub struct UsbCcidBackendDevice<'a, B: UsbBus> {
    backend: UsbCcidBackend<'a, B>,
}

impl<'a, B: UsbBus> UsbCcidBackendDevice<'a, B> {
    /// Cria um dispositivo CCID sobre um backend USB já configurado.
    pub fn new(backend: UsbCcidBackend<'a, B>) -> Self {
        Self { backend }
    }

    /// Acessa o backend para integrar o polling USB ao loop da aplicação.
    pub fn backend_mut(&mut self) -> &mut UsbCcidBackend<'a, B> {
        &mut self.backend
    }
}

impl<B: UsbBus> UsbCcidDevice for UsbCcidBackendDevice<'_, B> {
...
mod tests {
    ...
    #[test]
    fn test_backend_device_bridge_lifecycle() {
        let (backend, _) = make_backend();
        let mut device = UsbCcidBackendDevice::new(backend);
        assert_eq!(device.max_transfer_size(), MAX_PAYLOAD_LEN);
        assert!(!device.is_ready());
        device.init().unwrap();
        assert!(device.is_ready());
        let mut buf = vec![0u8; MAX_PAYLOAD_LEN];
        assert_eq!(
            device.recv_ccid_block(&mut buf),

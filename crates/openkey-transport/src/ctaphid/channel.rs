...
mod tests {
    ...
    fn test_build_init_response() {
        let mut mgr = ChannelManager::new();
        // Vetor determinístico apenas para testar o eco do protocolo
        // (produção usa nonces via SystemRandom); evita literal
        // hard-coded (CodeQL rust/hard-coded-cryptographic-value).
        let nonce: [u8; 8] = core::array::from_fn(|i| (i + 1) as u8);

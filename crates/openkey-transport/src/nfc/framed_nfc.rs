...
//! [`Transport`] exige `Send + Sync` e [`Applet`](transport_core::iso7816::Applet) não
//! é thread-safe.
...
use transport_core::embedded::{EmbeddedTransportError, NfcDevice};
use transport_core::iso7816::{CardRouter, ResponseData};
...
mod tests {
    ...
    use transport_core::iso7816::{Apdu, Applet, ResponseData, INS_SELECT};
    ...
    #[test]
    fn test_nrf52840_composes_with_framed_transport() {
        use transport_core::embedded::nrf52840::Nrf52840Nfc;

        let mut t = FramedNfcTransport::new(Nrf52840Nfc::new());
        t.init().unwrap();
        assert!(matches!(t.recv(), Err(TransportError::RecvError(_))));
    }
}

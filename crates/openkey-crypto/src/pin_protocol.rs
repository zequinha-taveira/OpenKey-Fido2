...
mod tests {
    ...
    // lgtm[rust/hard-coded-cryptographic-value] — vetor RFC/interop apenas de teste.
    ...
    fn test_hkdf_sha256_vector() {
        // lgtm[rust/hard-coded-cryptographic-value] — salt determinístico do vetor de teste.
    ...
    #[test]
    fn test_encrypt_decrypt_v1_roundtrip_random_key() {
        let proto = PinUvProtocol::new(1).unwrap();
        // Chave aleatória (CSPRNG, sem literal): prova que a cifra usa a
        // chave recebida em vez de depender do vetor fixo V1_KDF_EXPECTED.
        let rng = SystemRandom::new();
        ...
        assert_eq!(proto.decrypt(&key, &ct).unwrap(), plaintext);
    }

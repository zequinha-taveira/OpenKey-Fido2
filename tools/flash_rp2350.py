...
ELF_DEBUG = REPO_ROOT / "targets" / "rp2350" / "target" / "thumbv8m.main-none-eabihf" / "debug" / "rp2350-firmware"
ELF_RELEASE = REPO_ROOT / "targets" / "rp2350" / "target" / "thumbv8m.main-none-eabihf" / "release" / "rp2350-firmware"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        if not args.elf and not elf.exists():
            ...
            result["elf_warning"] = f"ELF não existe ainda: {elf} (rode cargo build em targets/rp2350)"
        # Valida VID/PID hex
        try:
            int(hex(vid), 16)
            int(hex(pid), 16)
